...
export default function LandlordMessages() {
  ...
  const loadThreads = async () => {
    ...
    const { data: applicationRows, error } = await supabase
      .from("applications")
      .select("id, property_id, tenant_id, status, message, updated_at")
            ...
            <div className="flex h-[560px] flex-col rounded-lg border bg-card">
              ...
              <div className="flex-1 space-y-3 overflow-y-auto p-4">
                {messages.map((message, index) => (
                  <div key={`${message.sender}-${index}`} className={`flex ${message.sender === "landlord" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[80%] rounded-xl px-3 py-2 text-sm ${message.sender === "landlord" ? "bg-primary text-primary-foreground" : "bg-accent text-foreground"}`}>
                      {message.text}
                    </div>
                  </div>
                ))}
