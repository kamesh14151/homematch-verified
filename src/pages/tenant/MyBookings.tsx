...
function DateRangeModal({
  ...
  const fmt = (d: Date | null) =>
    d ? d.toLocaleDateString("en-IN", { day: "2-digit", month: "2-digit", year: "2-digit" }) : "DD/MM/YY";
  ...
  const renderMonth = (year: number, month: number) => {
    ...
    return (
      <div className="flex-1 min-w-0">
        <div className="text-center text-sm font-bold text-[#0f2a5c] mb-3">{label} {year}</div>
            ...
            return (
              <button
                ...
                className={[
                  ...
                  isStart || isEnd ? "bg-[#0f2a5c] text-white rounded-full font-bold z-10" : "",
                  ranged && !isStart && !isEnd ? "bg-[#0f2a5c]/10 text-[#0f2a5c]" : "",
                  !ranged && !isStart && !isEnd ? "hover:bg-zinc-100 rounded-full text-zinc-700" : "",
                ].join(" ")}
              >
                {day.getDate()}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl p-6 relative">
        <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-full hover:bg-zinc-100 transition-colors">
        ...
        <div className="flex items-center justify-between mb-6 px-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[9px] font-bold text-yellow-500 uppercase tracking-widest">DATE FROM</p>
              <p className="text-sm font-bold text-[#0f2a5c]">{fmt(start)}</p>
            </div>
          </div>
          <div className="w-12 h-px bg-zinc-200" />
          <div className="flex items-center gap-2">
            <div className="text-right">
              <p className="text-[9px] font-bold text-yellow-500 uppercase tracking-widest">DATE TO</p>
              <p className="text-sm font-bold text-[#0f2a5c]">{fmt(end)}</p>
            </div>
            <Calendar className="w-4 h-4 text-yellow-500" />
          </div>
        </div>
        ...
        <div className="flex items-start gap-2">
          <button onClick={() => setBase(new Date(base.getFullYear(), base.getMonth() - 1, 1))} className="p-2 rounded-full hover:bg-zinc-100 mt-1 shrink-0">
        ...
        <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-zinc-100">
          <button onClick={() => { setStart(null); setEnd(null); }} className="px-5 py-2.5 rounded-full text-sm font-semibold text-zinc-600 hover:bg-zinc-100 transition-colors">
            Clear Dates
          </button>
          <button onClick={() => onConfirm(start, end)} className="px-6 py-2.5 rounded-full text-sm font-semibold bg-[#0f2a5c] text-white hover:bg-[#0f2a5c]/90 transition-colors">
            Confirm Dates
          </button>
        </div>
      </div>
...
function EmptyBookings() {
  return (
    <div className="flex flex-col items-center justify-center py-16">
      <svg width="110" height="120" viewBox="0 0 110 120" fill="none" className="mb-4">
        <path d="M8 55L98 14L72 95L52 65L8 55Z" stroke="#0f2a5c" strokeWidth="2.2" strokeLinejoin="round" />
        <path d="M52 65L73 46" stroke="#0f2a5c" strokeWidth="2.2" />
      ...
      <p className="text-[#0f2a5c] font-bold text-lg">No Booking Available !</p>
      <p className="text-muted-foreground text-sm mt-1">Future bookings will appear here</p>
    </div>
  );
}
...
export default function MyBookings() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const propertyIds = [...new Set(applications.map((item) => item.property_id))];
      ...
      const [{ data: properties }, { data: images }] = await Promise.all([
        propertyIds.length > 0
          ? supabase
              ...
              .in("id", propertyIds)
          : Promise.resolve({ data: [] as Array<{ id: string; title: string; address: string; rent: number; booking_hold_amount: number | null }> }),
        propertyIds.length > 0
          ? supabase
          ...
          : Promise.resolve({ data: [] as Array<{ property_id: string; image_url: string; display_order: number }> }),
      ]);
      ...
      const mapped: BookingRow[] = applications.map((item) => {
        ...
        const normalizedStatus: BookingRow["status"] =
          sourceStatus === "APPROVED" ? "paid" : sourceStatus === "REJECTED" ? "cancelled" : "pending";
        ...
        return {
          ...
          amount: property?.booking_hold_amount ?? Math.round((property?.rent ?? 0) * 0.25),
  ...
  const filtered = useMemo(() => {
    let rows = bookings;
    if (filter !== "ALL") rows = rows.filter((b) => b.status.toUpperCase() === filter);
    if (searchMode === "id" && idQuery.trim()) {
      const q = idQuery.toLowerCase().trim();
      rows = rows.filter((b) => b.id.toLowerCase().includes(q) || b.propertyTitle.toLowerCase().includes(q));
    }
  ...
  const dateRangeLabel = () => {
    ...
    const f = (d: Date) => d.toLocaleDateString("en-IN", { month: "short", day: "numeric", year: "numeric" });
    return `${f(dateRange.start)} \u2013 ${dateRange.end ? f(dateRange.end) : "?"}`;
  };
    ...
    <DashboardLayout navItems={navItems} title="Tenant">
      <div className="space-y-6 max-w-4xl mx-auto">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            <Link to="/tenant/dashboard" className="hover:underline">Home</Link>
        ...
        {/* Search card */}
        <div className="rounded-2xl bg-[#0f2a5c] p-6 text-white">
              ...
              <label key={value} className="flex items-center gap-2.5 cursor-pointer select-none">
                <button
                  ...
                  className={[
                    ...
                    searchMode === value ? "border-yellow-400 bg-yellow-400" : "border-white/30",
                  ].join(" ")}
                >
                  {searchMode === value && <span className="w-2 h-2 bg-white rounded-full" />}
                </button>
                <span onClick={() => setSearchMode(value)} className="text-sm">{label}</span>
              </label>
            ))}
          </div>
          ...
          {searchMode === "id" ? (
            <div className="flex gap-2">
              <Input
                ...
                className="bg-white/10 border-white/20 text-white placeholder:text-white/40 focus-visible:ring-yellow-400/50"
              />
              <Button size="icon" className="shrink-0 bg-yellow-400 text-[#0f2a5c] hover:bg-yellow-400/90">
        ...
        <div className="rounded-2xl border bg-card p-6">
            ...
            {(["ALL", "PENDING", "PAID"] as FilterTab[]).map((tab) => (
              <button
                ...
                className={[
                  ...
                  filter === tab ? "bg-[#0f2a5c] text-white" : "border border-border text-muted-foreground hover:border-muted-foreground/50",
                ].join(" ")}
              >
                {tab}
              </button>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {filtered.map((b) => {
                const thumb = b.imageUrl || `https://picsum.photos/seed/${b.propertyId}/400/300`;
                const statusLabel = b.status.charAt(0).toUpperCase() + b.status.slice(1);
                return (
                  <div key={b.id} className="flex flex-col sm:flex-row gap-4 p-4 rounded-xl border border-border hover:border-primary/30 hover:shadow-sm transition-all group">
                    {/* Thumb */}
                    ...
                    <div className="flex-1 space-y-1.5">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                        <div>
                          ...
                          {b.propertyAddress && (
                            <p className="text-xs text-muted-foreground">{b.propertyAddress}</p>
                          )}
                          <p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                            <Calendar className="h-3 w-3" /> Booked on {fmtDate(b.createdAt)}
                          </p>
                        </div>
                        <Badge
                      ...
                      <div className="flex gap-2 pt-1">
                        ...
                        {b.status === "pending" && (
                          <Link to={`/property/${b.propertyId}/book`}>
                            <Button size="sm" className="h-7 text-xs rounded-lg bg-[#0f2a5c] hover:bg-[#0f2a5c]/90">
                              Complete Payment
                            </Button>
                          </Link>
                        )}
      ...
      {showPicker && (
        <DateRangeModal
          ...
          onConfirm={(s, e) => { setDateRange({ start: s, end: e }); setShowPicker(false); }}
        />
      )}
    </DashboardLayout>
  );
