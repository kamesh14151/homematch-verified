...
export default function TenantDashboard() {
  ...
  // Location state (for near-you sorting + search prefill only)
  ...
  // City-matched properties sorted to top for the near-you preview
  ...
  // Search URL pre-filled with detected city
  const searchUrl = userLocation
    ? `/search?location=${encodeURIComponent(userLocation.city)}`
    : "/search";
      ...
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {/* ── Near You — compact 3-card preview; full search lives at /search ── */}
            <div className="overflow-hidden rounded-[1.75rem] border border-border/70 bg-background/82 shadow-[0_18px_44px_-36px_rgba(91,71,56,0.22)] dark:border-white/8 dark:bg-zinc-950/88">
              {/* Header */}
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/50 px-5 py-4 dark:border-white/8">
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="font-semibold text-foreground">Homes Near You</h2>
                    ...
                    {locationStatus === "ip" && userLocation && (
                      <span className="flex items-center gap-1 rounded-full border border-blue-200 bg-blue-50 px-2 py-0.5 text-[10px] font-medium text-blue-700 dark:border-blue-800/30 dark:bg-blue-950/20 dark:text-blue-400">
                        <Navigation className="h-2.5 w-2.5" /> IP location
                      </span>
                    )}
                    {locationStatus === "gps" && (
                      <span className="flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[10px] font-medium text-emerald-700 dark:border-emerald-800/30 dark:bg-emerald-950/20 dark:text-emerald-400">
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    {userLocation
                      ? `Showing verified homes near ${userLocation.city}`
                      : `${nearbyProperties.length} verified homes available`}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  {locationStatus === "ip" && (
                    <Button
                      ...
                      onClick={requestGPS}
                    >
                      <Locate className="mr-1.5 h-3 w-3" /> Enable GPS
                    </Button>
                  )}
                  <Link
                    to={searchUrl}
                  ...
                  >
                    Search near {userLocation?.city ?? "you"} <ArrowRight className="ml-1 h-3.5 w-3.5" />
                  </Link>
                </div>
              </div>
              ...
              {/* Footer CTA */}
              <div className="flex items-center justify-between border-t border-border/40 px-5 py-3 dark:border-white/8">
                <p className="text-xs text-muted-foreground">
                  {nearbyProperties.length > 3
                    ? `+${nearbyProperties.length - 3} more homes available`
                    : `${nearbyProperties.length} homes total`}
                </p>
                <Link
                  to={searchUrl}
                  className="flex items-center gap-1 text-xs font-semibold text-primary hover:underline"
                >
                  View all results <ArrowRight className="ml-0.5 h-3 w-3" />
                </Link>
              </div>
            </div>
