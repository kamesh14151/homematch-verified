...
import { Search, Bookmark, FileText, MessageSquare, UserCircle, Home, Send, Loader2 } from "lucide-react";
...
export default function TenantMessages() {
  ...
  const loadThreads = async () => {
    ...
    const [{ data: props }, { data: profiles }] = await Promise.all([
      supabase.from("properties").select("id, title, landlord_id").in("id", propertyIds),
      supabase.from("profiles").select("user_id, full_name"),
    ]);

    const propMap = new Map((props ?? []).map((p) => [p.id, p]));
    const profileMap = new Map((profiles ?? []).map((p) => [p.user_id, p.full_name || "Landlord"]));
    ...
    const mapped = apps.map((a) => {
      ...
      return {
        ...
        landlordName: prop ? profileMap.get(prop.landlord_id) || "Landlord" : "Landlord",
    ...
    <DashboardLayout navItems={navItems} title="Tenant">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Chat with landlords in real-time</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="rounded-lg border bg-card p-8 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Start a conversation by applying to a property</p>
          </div>
        ) : (
